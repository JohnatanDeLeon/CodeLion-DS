 ...
 * ```
 */
export function getMediaQuery(breakpoint: keyof typeof designSystemConfig.breakpoints): string {
...
export function createResponsiveValue<T>(
  ...
  Object.entries(values).forEach(([breakpoint, value]) => {
    if (value !== undefined) {
      result[getMediaQuery(breakpoint as keyof typeof designSystemConfig.breakpoints)] = value;
    }
  });
 ...
 */
export type ResponsiveValue<T> = T | Partial<Record<keyof typeof designSystemConfig.breakpoints, T>>;
...
export function resolveResponsiveValue<T>(
  ...
  const responsiveValue = value as Partial<Record<keyof typeof designSystemConfig.breakpoints, T>>;
  
  // If no current breakpoint specified, return the largest available value
  if (!currentBreakpoint) {
    const breakpoints = Object.keys(designSystemConfig.breakpoints) as Array<keyof typeof designSystemConfig.breakpoints>;
  ...
  return responsiveValue.xs ?? Object.values(responsiveValue)[0] as T;
}
...
export const responsive = {
   ...
   */
  spacing: (values: Partial<Record<keyof typeof designSystemConfig.breakpoints, string>>) =>
    createResponsiveValue(values),
    
  /**
   * Creates responsive typography
   */
  fontSize: (values: Partial<Record<keyof typeof designSystemConfig.breakpoints, string>>) =>
    createResponsiveValue(values),
} as const;