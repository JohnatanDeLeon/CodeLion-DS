...
describe('a11y utilities', () => {
  ...
  describe('focusRing', () => {
    ...
    it('should accept custom color', () => {
      ...
      expect(result[':focus-visible'].boxShadow).toBe('0 0 0 2px rgb(255 0 0)80');
    });

    it('should accept custom opacity', () => {
      const result = focusRing('rgb(0 255 0)', 1.0);
      
      expect(result[':focus-visible'].boxShadow).toBe('0 0 0 2px rgb(0 255 0)ff');
    });

    it('should handle zero opacity', () => {
      const result = focusRing('rgb(255 255 255)', 0);
      
      expect(result[':focus-visible'].boxShadow).toBe('0 0 0 2px rgb(255 255 255)0');
    });

    it('should handle fractional opacity correctly', () => {
      const result = focusRing('rgb(100 150 200)', 0.25);
      
      expect(result[':focus-visible'].boxShadow).toBe('0 0 0 2px rgb(100 150 200)40');
    });
  });
  ...
  describe('contrast.getContrastRatio', () => {
    ...
    it('should return 1 for identical colors', () => {
      const result = contrast.getContrastRatio([128, 128, 128], [128, 128, 128]);
      expect(result).toBe(1);
    });
    ...
    it('should calculate reasonable ratios for common colors', () => {
      ...
      const blueOnWhite = contrast.getContrastRatio([0, 0, 255], [255, 255, 255]);
  ...
  describe('announce.message', () => {
    it('should create and cleanup live region', () => {
      ...
      expect(() => announce.message('Assertive message', 'assertive')).not.toThrow();
    });
  });
});
